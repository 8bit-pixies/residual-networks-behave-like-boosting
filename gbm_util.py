...
def rf_model2(dim_size, n_tree=5, depth=5, sample=0.7, num_class=8, last_only=False):
    main_input = Input(shape=(dim_size,), name='main_input')
    # this is where the decision trees are learned
    trees = [dnrf2(main_input, idx, depth, num_class, dim_size, sample) for idx in range(n_tree)]
    ...
    pred_trees = [x[0] for x in trees]
    stack_pred = layers.Lambda(normalise_pred, output_shape=normalise_pred_shape, name=f'pred_layer_rf')(pred_trees)
    ...
    if last_only:
        ...
        outputs = list(chain.from_iterable(trees))+[stack_pred]
        print("Num outputs: {}".format(len(outputs)))
        model = Model(inputs=[main_input], outputs=outputs)
        return model, len(outputs)


def gbm2(x, tree_num=1, depth=4, num_class=2, dim_size=None, sample=0.7):
    # to do, implement random column sampling/filtering
    # via lambda layer or otherwise
    if dim_size is not None:
        sample_size = min(max(int(dim_size * sample), 1), dim_size)
    ...
    leaf_layers = layers.Lambda(lambda x: [tf.squeeze(y) for y in tf.split(x, [1 for _ in range(K.int_shape(x)[2])], axis=2)], output_shape=outputshape)(tree_route)
    leaves_layer_tree = [Dense(num_class, name=f'decision_leaf_{tree_num}_{idx}')(x) for idx, x in enumerate(leaf_layers)]
    pred_layer_tree = [layers.Activation('softmax', name=f'decision_tree_sm_{tree_num}_{idx}')(x) for idx, x in enumerate(leaves_layer_tree)]
    next_layer_tree = [layers.Activation('relu', name=f'decision_tree_rl_{tree_num}_{idx}')(x) for idx, x in enumerate(leaves_layer_tree)]
    ...
    stack_pred = layers.Lambda(normalise_pred, output_shape=normalise_pred_shape, name=f'pred_layer_{tree_num}')(pred_layer_tree)
    stack_next = layers.Lambda(normalise_pred, output_shape=normalise_pred_shape, name=f'stack_layer_{tree_num}')(next_layer_tree)
    embedding = Dense(dim_size, name=f'tree_embed_{tree_num}')(Flatten()(stack_next))

    test_out = DecisionPredRouting(depth=depth)([stack_pred, tree_nodes])
    return embedding, [test_out]


def gbm_model2(dim_size, n_tree=3, depth=3, sample=0.7, num_class=10, last_only=False):
    from functools import reduce
    main_input = Input(shape=(dim_size,), name='main_input')
    # this is where the decision trees are learned
    ...
    for idx in range(n_tree):
        if idx == 0 :
            embed_x, p_out = gbm2(main_input, idx, depth, num_class, dim_size, sample)
            y = ShrinkageFactor(1.0, is_trainable=True, name=f"shrinkage_{idx}")(main_input)
            x = layers.add([embed_x, y])
            pred_out.extend(p_out)
        else:
            embed_x, p_out = gbm2(x, idx, depth, num_class, dim_size, sample)
            y = ShrinkageFactor(1.0, is_trainable=True, name=f"shrinkage_{idx}")(x)
            x = layers.add([embed_x, y])
            pred_out.extend(p_out)

    print("Pred output is of length: {}".format(len(pred_out)))
    if last_only:
        model = Model(inputs=[main_input], outputs=pred_out[-1])
        return model, 1
    else:
        model = Model(inputs=[main_input], outputs=pred_out)
        return model, len(pred_out)